...
export function buildOvertakeLayer(
  ...
  if (geometry.detectionM !== null && Number.isFinite(geometry.detectionM)) {
    // Solid and bright: this one is a measured position of a stated rule.
    const gate = buildGate(track, geometry.detectionM, {
      colour: HAAS.white, dashed: false, opacity: 0.95,
    });
    if (gate) {
  ...
  for (const zone of geometry.zones) {
    ...
    // Dashed and dimmer: a historical-DRS proxy, not a surveyed 2026 line.
    const gate = buildGate(track, zone.activationM, {
      colour: HAAS.red, dashed: true, opacity: 0.8,
    });
    if (gate) {
